...
const generateMockMaintenanceRequests = (): MaintenanceRequest[] => {
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    requests.push({
      ...
      issueFrequency: i % 3 === 0 ? 'PERMANENT' : i % 3 === 1 ? 'FREQUENT' : 'RARE',
...
const maintenanceSlice = createSlice({
  ...
  reducers: {
    ...
    fetchMaintenanceRequestsSuccess: (state, action: PayloadAction<MaintenanceRequest[]>) => {
    ...
    setCurrentMaintenanceRequest: (state, action: PayloadAction<MaintenanceRequest | null>) => {
      state.currentRequest = action.payload;
    },
    filterMaintenanceRequests: (state, action: PayloadAction<{ resourceId?: string; status?: MaintenanceStatus; technicianId?: string }>) => {
    ...
    addMaintenanceRequest: (state, action: PayloadAction<MaintenanceRequest>) => {
      state.requests.push(action.payload);
      state.filteredRequests = state.requests;
    },
    updateMaintenanceRequest: (state, action: PayloadAction<MaintenanceRequest>) => {
    ...
    assignTechnician: (state, action: PayloadAction<{ requestId: string; technicianId: string }>) => {
    ...
    resolveMaintenanceRequest: (state, action: PayloadAction<{ requestId: string; resolution: string }>) => {
    ...
    returnToSupplier: (state, action: PayloadAction<{ requestId: string; reason: string }>) => {
