...
const generateMockResources = (): Resource[] => {
  ...
  for (let i = 1; i <= 15; i++) {
    resources.push({
      ...
      status: i % 10 === 0 ? 'MAINTENANCE' : i % 7 === 0 ? 'DISPOSED' : i < 10 ? 'ASSIGNED' : 'AVAILABLE',
...
const resourcesSlice = createSlice({
  ...
  reducers: {
    ...
    filterResources: (state, action: PayloadAction<{ departmentId?: string; type?: ResourceType; status?: string }>) => {
    ...
    updateResource: (state, action: PayloadAction<Resource>) => {
      ...
      if (index !== -1) {
        ...
        const filteredIndex = state.filteredResources.findIndex(r => r.id === action.payload.id);
        if (filteredIndex !== -1) {
    ...
    removeResource: (state, action: PayloadAction<string>) => {
      ...
      state.filteredResources = state.filteredResources.filter(r => r.id !== action.payload);
