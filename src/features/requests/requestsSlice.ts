...
const generateMockRequests = (): ResourceRequest[] => {
  ...
  for (let i = 1; i <= 15; i++) {
    ...
    requests.push({
      ...
      updatedAt: new Date(
        createdDate.getTime() + 1000 * 60 * 60 * 24 * (i % 3)
      ).toISOString(),
...
const requestsSlice = createSlice({
  ...
  reducers: {
    ...
    setCurrentRequest: (
      state,
      action: PayloadAction<ResourceRequest | null>
    ) => {
      state.currentRequest = action.payload;
    },
    filterRequests: (
      state,
      action: PayloadAction<{ departmentId?: string; status?: RequestStatus }>
    ) => {
    ...
    removeRequest: (state, action: PayloadAction<string>) => {
      ...
      state.filteredRequests = state.filteredRequests.filter(
