        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="truncate">
              {request.requestedProducts.length} article
              {request.requestedProducts.length > 1 ? "s" : ""}
            </span>
          </div>
            ...
            >
              <div className="pt-4 space-y-4 mt-2 border-t">
                <div>
                  ...
                  <div className="space-y-2">
                    {request.requestedProducts.map(
                      (item: any, index: number) => (
                        <div
                          key={index}
                          className="pl-2 border-l-2 border-l-primary/20 text-sm"
                        >
                          ...
                          <p className="text-xs text-muted-foreground pl-1">
                            {item.type === ResourceType.COMPUTER ? (
                              // ? `${item.brand} ${item.specifications.cpu} ${item} ${""}`
                              <>
                                <span>Marque: {item.brand}</span>
                                <br />
                                <span>CPU: {item.specifications.cpu}</span>
                                <br />
                                <span>RAM: {item.specifications.ram}</span>
                                <br />
                                <span>
                                  Disque dure: {item.specifications.storage}
                                </span>
                                <br />
                                <span>
                                  Ecran: {item.specifications.monitor}
                                </span>
                              </>
                            ) : (
                              <>
